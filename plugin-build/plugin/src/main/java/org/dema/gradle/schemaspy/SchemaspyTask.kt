...
abstract class SchemaspyTask : DefaultTask() {
    ...
    fun sampleAction() {
        ...
        postgresContainer.use { postgres ->
            ...
            GenericContainer<Nothing>(SCHEMASPY_IMAGE).apply {
                ...
                schemaSpy.start()
                val generateDocCommand = schemaSpy.execInContainer(
                    "java",
                    "-jar", "/schemaspy-6.1.0.jar",
                    "-t", "pgsql11",
                    "-db", postgresContainer.databaseName,
                    "-host", "postgres",
                    "-u", postgresContainer.username,
                    "-p", postgresContainer.password,
                    "-o", "/output",
                    "-dp", "/drivers_inc",
                    "-I", excludeTables.get(),
                    "-debug"
    ...
    private fun Connection.database(): Database? = DatabaseFactory.getInstance()
